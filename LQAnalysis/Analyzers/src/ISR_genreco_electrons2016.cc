// $Id: ISR_genreco_electrons2016.cc 1 2013-11-26 10:23:10Z jalmond $
/***************************************************************************
 * @Project: LQISR_genreco_electrons2016 Frame - ROOT-based analysis framework for Korea SNU
 * @Package: LQCycles
 *
...
/// Local includes
#include "ISR_genreco_electrons2016.h"
...
ClassImp (ISR_genreco_electrons2016);
  ...
  *
  */
ISR_genreco_electrons2016::ISR_genreco_electrons2016() :  AnalyzerCore(), out_electrons(0)  {
  
  
  // To have the correct name in the log:                                                                                                                            
  SetLogName("ISR_genreco_electrons2016");
  
  Message("In ISR_genreco_electrons2016 constructor", INFO);
  //
...
void ISR_genreco_electrons2016::InitialiseAnalysis() throw( LQError ) {
...
void ISR_genreco_electrons2016::ExecuteEvents()throw( LQError ){
 ...
 // pileup_reweight = mcdata_correction->PileupWeightByPeriod(eventbase->GetEvent());
  ...
  TString dielectron_trigelectron_trig1="HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL_DZ_v";//
  ...
  TString single_trig1 = "HLT_Ele27_WPTight_Gsf";
     //"HLT_IsoMu24_v"; // 
  ...
  trignames.push_back(dielectron_trigelectron_trig1);
   ...
   */
   ...
   //   double triggersf_up =  TriggerSF_dimu_jhchoi(electrons,trignames2[0],1);//
double triggersf_up = mcdata_correction->TriggerScaleFactor(electrons, muons, trignames2[0],1);                                                             
//     double triggersf_down = TriggerSF_dimu_jhchoi(electrons,trignames2[0],-1);// 
  ...
  //double MCDataCorrections::TriggerScaleFactor( vector<snu::KElectron> el, vector<snu::KMuon> mu,  TString trigname, int direction){
   ...
   //   double iso04_0 = electrons.at(0).RelIso04();
   //double iso04_1 = es.at(1).RelIso04();
   ...
   bool mcfromtau1 = electrons[0].MCFromTau();
   bool mcfromtau2 = electrons[1].MCFromTau();
   bool TriggerMatch1 = (electrons[0].TriggerMatched(dielectron_trigelectron_trig1) && electrons[1].TriggerMatched(dielectron_trigelectron_trig1));
   ...
   //if(!TriggerMatch1 &&!TriggerMatch2 &&!TriggerMatch3 && TriggerMatch4) return;
    bool SingleTriggerMatch1 = (electrons[0].TriggerMatched(single_trig1)|| electrons[1].TriggerMatched(single_trig1));
   //   bool SingleTriggerMatch1 = (electrons[0].TriggerMatched(single_trig1));
   // bool SingleTriggerMatch2 = TriggerMatch(single_trig2,muons);
   ...
   if((ptlep1>10) && (ptlep2>5) && (fabs(etalep1) < 3) &&( fabs(etalep2) <3)){
     ...
     double corpriptstat[100], corsecptstat[100];
     for(int i = 0; i <100; i ++){

       //       corpriptstat[i]= mcdata_correction->GetCorrectedMuonMomentum(muons[0],eventbase->GetTruth(),1,i);
       // corsecptstat[i]= mcdata_correction->GetCorrectedMuonMomentum(muons[1],eventbase->GetTruth(),1,i);
       //       cout<<"corpriptstat["<<i<<"]="<<corpriptstat[i]<<endl;                                                                        
       corpriptstat[i]=0;
       corsecptstat[i]=0;

     }

     FillNtp("corpriptstat", corpriptstat);
     FillNtp("corsecptstat", corsecptstat);

     double corpriptzpt[1], corsecptzpt[1];
     //corpriptzpt[0]= mcdata_correction->GetCorrectedMuonMomentum(muons[0],eventbase->GetTruth(),2,0);
     // corsecptzpt[0]= mcdata_correction->GetCorrectedMuonMomentum(muons[1],eventbase->GetTruth(),2,0);
     corpriptzpt[0]=0;
     corsecptzpt[1]=0;

     FillNtp("corpriptzpt", corpriptzpt);
     FillNtp("corsecptzpt", corsecptzpt);

     double corpriptpmw[5], corsecptpmw[5];
     for(int i = 0; i <5; i ++){
       // corpriptpmw[i]= mcdata_correction->GetCorrectedMuonMomentum(muons[0],eventbase->GetTruth(),4,i);
       //corsecptpmw[i]= mcdata_correction->GetCorrectedMuonMomentum(muons[1],eventbase->GetTruth(),4,i);
       // cout<<"corpriptpmw["<<i<<"]="<<corpriptpmw[i]<<endl;                                                                                
       corpriptpmw[i]=0;
       corsecptpmw[i]=0;

     }

     FillNtp("corpriptpmw", corpriptpmw);
     FillNtp("corsecptpmw", corsecptpmw);

     double corpriptfmw[5], corsecptfmw[5];
     for(int i = 0; i <5; i ++){
       //corpriptfmw[i]= mcdata_correction->GetCorrectedMuonMomentum(muons[0],eventbase->GetTruth(),5,i);
       //corsecptfmw[i]= mcdata_correction->GetCorrectedMuonMomentum(muons[1],eventbase->GetTruth(),5,i);
       // cout<<"corpriptfmw["<<i<<"]="<<corpriptfmw[i]<<endl;                                                                                 
       corpriptfmw[i]=0;
       corsecptfmw[i]=0;

     }

     FillNtp("corpriptfmw", corpriptfmw);
     FillNtp("corsecptfmw", corsecptfmw);


     double lep1[16], lep2[16], weights[18], othervs[22];
     double jetinfo[3];
     lep1[8]=electrons[0].Pt();
     lep1[9] = electrons[0].Eta();
     lep1[10] = electrons[0].Phi();
     lep1[11] = electrons[0].E();
     lep1[12] = electrons[0].Charge();
     // lep1[13] = electrons[0].ActiveLayer();
     lep1[13] = 0;
     lep1[14] = electrons[0].PtShiftedUp();
     lep1[15] = electrons[0].PtShiftedDown();

     lep2[8]=electrons[1].Pt();
     lep2[9] = electrons[1].Eta();
     lep2[10] = electrons[1].Phi();
     lep2[11] = electrons[1].E();
     lep2[12] = electrons[1].Charge();
     //     lep2[13] = electrons[1].ActiveLayer();
     lep2[13] = 0;
     lep2[14] = electrons[1].PtShiftedUp();
     lep2[15] = electrons[1].PtShiftedDown();
     float metx_old= eventbase->GetEvent().PFMETx();
     float mety_old= eventbase->GetEvent().PFMETy();
     float met_old= eventbase->GetEvent().PFMET();
     float pxold = (electrons[0]+electrons[1]).Px();
     float pyold = (electrons[0]+electrons[1]).Py();
     //mcdata_correction->CorrectElectronMomentum(electrons,eventbase->GetTruth());
     float pxnew = (electrons[0]+electrons[1]).Px();
     float pynew = (electrons[0]+electrons[1]).Py();
     ...
     lep1[0] = electrons[0].Pt();
     lep1[1] = electrons[0].Eta();
     lep1[2] = electrons[0].Phi();
     lep1[2] = electrons[0].Phi();
     lep1[3] = electrons[0].E();
     lep1[4] = electrons[0].Charge();
     lep1[5] = mcfromtau1;
     lep1[6] = 1;
     lep1[7] = 1;


     lep2[0] = electrons[1].Pt();
     lep2[1] = electrons[1].Eta();
     lep2[2] = electrons[1].Phi();
     lep2[2] = electrons[1].Phi();
     lep2[3] = electrons[1].E();
     lep2[4] = electrons[1].Charge();
     lep2[5] = mcfromtau1;
     lep2[6] = 1;
     lep2[7] = 1;
     ...
     FillHist("Mass",dimass,weight*idsf*pileup_reweight*triggersf*recosf,80,100,20);
...
void ISR_genreco_electrons2016::EndCycle()throw( LQError ){
...
void ISR_genreco_electrons2016::BeginCycle() throw( LQError ){
...
ISR_genreco_electrons2016::~ISR_genreco_electrons2016() {
  
  Message("In ISR_genreco_electrons2016 Destructor" , INFO);
  
}


void ISR_genreco_electrons2016::BeginEvent( )throw( LQError ){
...
void ISR_genreco_electrons2016::MakeHistograms(){
  ...
  /**
   *  Remove//Overide this ISR_genreco_electrons2016Core::MakeHistograms() to make new hists for your analysis
   **/
  ...
  //$%^//                                                                                                                                                     
  MakeNtp("weights", "weight:MCweight:idsf:pu:triggersf:ps:recosf:idsf_up:idsf_down:recosf_up:recosf_down:pu_up:pu_down:pu69:pu69_up:pu69_down:triggersf_up:triggersf_down");
...
void ISR_genreco_electrons2016::ClearOutputVectors() throw(LQError) {
